'use strict';
// added aqara lock s2 support, base on https://github.com/Koenkk/zigbee-shepherd-converters
const debug = require('debug')('zigbee-shepherd-converters:devices');
const fz = require('./converters/fromZigbee');
const tz = require('./converters/toZigbee');

const INVALID_OPEN = "65297";
const VALID_OPEN = "65296";
const DOOR_STATUS = "65526";
...
const lockfz = {
    doorlock_report: {
        ...
        convert: (model, msg, publish, options) => {
            ...
            var status_str = "";
            if (INVALID_OPEN in data) {
                var payload = new Buffer(data[INVALID_OPEN]);
                const wrong_counter = payload[3];
                return {
                    "invalid_tries": wrong_counter,
                    "status": new Date().toLocaleString() + ":" + "开门错误{0}次".format(wrong_counter),
                    // "data": data[INVALID_OPEN]
                };
            } else if (VALID_OPEN in data) {
                payload = new Buffer(data[VALID_OPEN]);
                const finger_no = payload[5];
                var who = "";
                for (const key in fingers) {
                    if (fingers[key].indexOf(finger_no) != -1) {
                        who = key;
                    }
                }
                return {
                    "valid_fingerprint": finger_no,
                    "status": new Date().toLocaleString() + ":" + "{0}的指纹验证通过".format(who),
                    // "data": data[VALID_OPEN]
                };
            } else if (DOOR_STATUS in data) {
                payload = new Buffer(data[DOOR_STATUS]);
                const action = payload[3];
                const lock_status = payload[4];
                var safelock, lock;
                safelock = lock = "off"; 
                switch (lock_status) {
                    case 0x00:
                ...
                return {
                    "status": new Date().toLocaleString() + ":" + status_str,
                    "lock": lock,
                    "safelock": safelock,
                    // "data": data[DOOR_STATUS]
                };
            }
            return data;
        },
    },
    doorlock_change_ignore: {
        cid: 'closuresDoorLock',
        type: 'devChange',
...
const devices = [
    // Xiaomi
    {
        ...
        model: 's2',
        vendor: 'Xiaomi',
        description: 'aqara s2',
        supports: 'aqara lock s2',
        fromZigbee: [lockfz.doorlock_report, lockfz.doorlock_change_ignore, lockfz.basic_report_ignore, lockfz.basic_change_ignore],
        toZigbee: [],
    },
    {
